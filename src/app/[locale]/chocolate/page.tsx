...
export default async function ChocolatePage() {
    const t = await getTranslations('ChocolatePage');

    return (
        <main className="min-h-screen pt-32 pb-24 px-6 md:px-12 flex flex-col items-center bg-[#08150a]">
            <div className="max-w-7xl w-full flex flex-col gap-16">
                {/* Header */}
                <header className="flex flex-col gap-6 max-w-2xl">
                    <span className="text-xs font-medium tracking-[0.4em] uppercase text-gold">Bean-to-Bar</span>
                    ...
                    {[1, 2, 3].map((i) => (
                        <div key={i} className="glass p-8 flex flex-col gap-6 group hover:scale-[1.02] transition-all duration-500">
                            <div className="aspect-[4/5] bg-cream/5 border border-cream/10 relative flex items-center justify-center">
                                <span className="text-[10px] tracking-[0.2em] text-cream/20 uppercase">Collection Image</span>
                            </div>
                            <div className="flex flex-col gap-2">
                                <h3 className="text-xl serif text-cream italic">Collection {i}</h3>
                                <p className="text-sm text-cream/50 font-light">Artisanal chocolate collection details and flavor percentage.</p>
                            </div>
                        </div>
                    ))}
                </div>
